...
public abstract class MeteorRenderPipelines {
    ...
    public static void precompile() {
        ...
        for (RenderPipeline pipeline : PIPELINES) {
            device.precompilePipeline(pipeline, (identifier, shaderType) -> {
                try (var in = openResource(resources, identifier)) {
    ...
    private static InputStream openResource(ResourceManager resources, net.minecraft.util.Identifier identifier) throws IOException {
        var resource = resources.getResource(identifier);
        if (resource.isPresent()) return resource.get().getInputStream();
        String path = "/assets/" + identifier.getNamespace() + "/" + identifier.getPath();
        InputStream input = MeteorRenderPipelines.class.getResourceAsStream(path);
        if (input == null) throw new IOException("Missing shader " + identifier);
        return input;
    }
